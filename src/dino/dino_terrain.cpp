...
void DinoTerrain::Init(DinoVec2 sizeOfTerrainInTiles, int NbrOfFlowersPerGroup, int season)
{
    DinoVec2 debug = XDino_GetRenderSize();
    m_translation = {(XDino_GetRenderSize().x - sizeOfTerrainInTiles.x * 16) / 2,
                     (XDino_GetRenderSize().y - sizeOfTerrainInTiles.y * 16) / 2};

    m_flwrNbrPerGroup = NbrOfFlowersPerGroup;
    m_season = season;
    ...
    m_sizeInTiles = sizeOfTerrainInTiles;
    vbufID_terrain = GenTerrainVertexBuffer();
}

DinoVec2 DinoTerrain::GetTopLeft()
{
    return {m_translation.x + 16, m_translation.y + 16}; // Corner tile that is not a border tile
}

DinoVec2 DinoTerrain::GetBottomRight()
{
    return {m_translation.x + (m_sizeInTiles.x - 1) * 16, m_translation.y + (m_sizeInTiles.y - 1) * 16};
...
uint64_t DinoTerrain::GenTerrainVertexBuffer()
{
    ...
    possiblePos.resize(m_sizeInTiles.x * m_sizeInTiles.y);

    for (int i = 0; i < m_sizeInTiles.x * m_sizeInTiles.y; i++) {
        float intPart;
        modf(i / m_sizeInTiles.x, &intPart);

        possiblePos[i] = {(float)(i % (int)m_sizeInTiles.x), intPart};
    }

    DinoVec2 uvBase;
    std::vector<DinoVertex> vs;
    vs.resize(6 * m_sizeInTiles.x * m_sizeInTiles.y);

    std::vector<DinoVec2> flwrPos;
    flwrPos.resize(m_flwrNbrPerGroup * 3);
    std::vector<DinoVec2> flwrUVs;
    flwrUVs.resize(m_flwrNbrPerGroup * 3);
    int flwr1Count = m_flwrNbrPerGroup;
    int flwr2Count = m_flwrNbrPerGroup;
    int flwr3Count = m_flwrNbrPerGroup;

    for (int i = 0; i < m_sizeInTiles.x * m_sizeInTiles.y; i++) {
        int indexOfTileToChange = XDino_RandomInt32(i, m_sizeInTiles.x * m_sizeInTiles.y - 1);
        ...
        // UV Base
        {
            ...
            else if (possiblePos[i].x == 0 && possiblePos[i].y == m_sizeInTiles.y - 1) {
                uvBase = {0, 48};
            }
            else if (possiblePos[i].x == m_sizeInTiles.x - 1 && possiblePos[i].y == 0) {
                uvBase = {32, 16};
            }
            else if (possiblePos[i].x == m_sizeInTiles.x - 1 && possiblePos[i].y == m_sizeInTiles.y - 1) {
                uvBase = {32, 48};
            }
            else if (possiblePos[i].x == 0) {
                uvBase = {0, 32};
            }
            else if (possiblePos[i].x == m_sizeInTiles.x - 1) {
                uvBase = {32, 32};
            }
            else if (possiblePos[i].y == 0) {
                uvBase = {16, 16};
            }
            else if (possiblePos[i].y == m_sizeInTiles.y - 1) {
                uvBase = {16, 48};
            }
...
{
    XDino_Draw(vbufID_terrain, texID_terrain, m_translation);
}
...
{
    XDino_Draw(vbufID_flwrs, texID_terrain, m_translation);
}