...
int APIENTRY WinMain(HINSTANCE hInst, HINSTANCE hInstPrev, PSTR cmdline, int cmdshow)
{
    ...
    // Fonction qui communique avec le système d'exploitation pour créer la fenêtre graphique principale de rendu.
    gXDino_hInstance = hInst;
    XDino_Win64_CreateWindow();
    ShowWindow(gXDino_hWindow, SW_SHOWNORMAL);
    UpdateWindow(gXDino_hWindow);

    // Boucle principale d'événement, qui traite les messages que le système d'exploitation nous envoit. Tourne en boucle tant que le programme continue.
    MSG msg;
    BOOL fGotMessage;
    ...
    // L'utilisateur a demandé à détruire la fenêtre. On notifie le système d'exploitation qu'on veut arrêter.
    Dino_GameShut();
...
{
    if (uMsg == WM_CREATE) {
        // La fenêtre graphique vient d'être construite, on initialise des variables globales et le moteur de rendu.
        gXDino_hWindow = hWnd;
    ...
    if (uMsg == WM_SIZE) {
        // L'utilisateur redimensionne la fenêtre. On ajuste le moteur de rendu en conséquence.
        XDino_Win64_ResizeRenderer(LOWORD(lParam), HIWORD(lParam));
        return 0;
    }
    if (uMsg == WM_PAINT) {
        // Le système d'exploitation demande à redessiner la fenêtre. On demande à la logique de jeu de dessiner une frame.
