...
void LobbyState::EnterState(double timeSinceStart)
{
    ...
    m_trees.emplace_back(offset + DinoVec2{50, 40}, 0, texId_terrain, m_dinoGameState, timeSinceStart);
    m_dinoGameState->g_dinoEntityManager.AddEntity(&m_trees.back());

    m_trees.emplace_back(offset + DinoVec2{100, 40}, 1, texId_terrain, m_dinoGameState, timeSinceStart);
    m_dinoGameState->g_dinoEntityManager.AddEntity(&m_trees.back());

    m_trees.emplace_back(offset + DinoVec2{150, 40}, 2, texId_terrain, m_dinoGameState, timeSinceStart);
    m_dinoGameState->g_dinoEntityManager.AddEntity(&m_trees.back());

    m_trees.emplace_back(offset + DinoVec2{200, 40}, 3, texId_terrain, m_dinoGameState, timeSinceStart);
    m_dinoGameState->g_dinoEntityManager.AddEntity(&m_trees.back());
}
...
void LobbyState::UpdateState(float deltaTime, double timeSinceStart)
{
    ...
    for (DinoGamepadIdx gamepadIdx : DinoGamepadIdx_ALL) {
        ...
        if (!m_dinoGameState->gamepadDino_map.contains(gamepadIdx)
            ...
            m_dinoGameState->g_scoreManager.AddPlayer(it->second.dinoID);
        }
        ...
        if (m_dinoGameState->gamepadDino_map.contains(gamepadIdx)
            ...
            m_dinoGameState->g_scoreManager.RemovePlayer(player.dinoID);
...
int LobbyState::FindUnusedId()
{
    std::vector<int> lookingId = {0, 1, 2, 3};
    for (int id : lookingId) {
        bool isValid = true;
        for (std::pair<const DinoGamepadIdx, dino_player>& gamepad_dino : m_dinoGameState->gamepadDino_map) {
            if (gamepad_dino.second.dinoID == id) {
                isValid = false;
                break;
            }
        }
